...
async function runE2E() {
  ...
  const chatModule =
    await import("file:///tmp/e2e-webui/webui/js/views/chat.js");
  ...
  function dispatchEvent(msg) {
    ...
    switch (type) {
      ...
      case "context_compressed":
        ...
        break;
      case "error":
        chat._onError(turnId, msg.message || "");
        break;
      ...
      case "input_attachments":
        ...
        break;
      default:
        console.warn(`[e2e] unhandled: ${type}`);
    }
  }
  ...
  assert(
    userBubble && userBubble.textContent.includes("分析 users 表"),
    "气泡文本应包含用户消息",
  );
  assert(
  ...
  const thinkingBody =
    thinkingCard && thinkingCard.querySelector(".thinking-body");
  assert(
  ...
  dispatchEvent({
    ...
    result: JSON.stringify({
      success: true,
      data: { content: "CREATE TABLE users (id, name, email)" },
    }),
  });
  ...
  dispatchEvent({
    ...
    toolName: "run_sql",
    summary: "ALTER TABLE",
    arguments: '{"sql":"ALTER TABLE users ADD COLUMN email_verified BOOLEAN"}',
  });
  dispatchEvent({
    type: "tool_call_complete",
    turnId: tid,
  ...
  const idxText1 = children.findIndex(
    (c) =>
      c.classList.contains("text-stream-segment") &&
      c.textContent.includes("我来"),
  );
  const idxTool1 = children.findIndex(
    (c) =>
      c.classList.contains("tool-card") &&
      c.querySelector(".tool-name")?.textContent === "read_file",
  );
  const idxText2 = children.findIndex(
    (c) =>
      c.classList.contains("text-stream-segment") &&
      c.textContent.includes("读完了"),
  );
  const idxTool2 = children.findIndex(
    (c) =>
      c.classList.contains("tool-card") &&
      c.querySelector(".tool-name")?.textContent === "run_sql",
  );
  const idxText3 = children.findIndex(
    (c) =>
      c.classList.contains("text-stream-segment") &&
      c.textContent.includes("已添加"),
  );

  console.log(
    `  DOM 顺序: text1=${idxText1}, tool1=${idxTool1}, text2=${idxText2}, tool2=${idxTool2}, text3=${idxText3}`,
  );
  assert(idxText1 >= 0, "text1 应在 DOM 中");
  assert(
    idxTool1 > idxText1,
    `工具1 (${idxTool1}) 必须在 text1 (${idxText1}) 之后 — 关键回归! 修复前: 工具堆在回答最末尾`,
  );
  assert(
    idxText2 > idxTool1,
    `text2 (${idxText2}) 必须在工具1 (${idxTool1}) 之后`,
  );
  assert(
    idxTool2 > idxText2,
    `工具2 (${idxTool2}) 必须在 text2 (${idxText2}) 之后`,
  );
  assert(
    idxText3 > idxTool2,
    `text3 (${idxText3}) 必须在工具2 (${idxTool2}) 之后`,
  );

  // 工具完成后默认折叠
  const tool1Body = tools[0] && tools[0].querySelector(".tool-body");
  assert(
    tool1Body && !tool1Body.classList.contains("open"),
    "工具1 完成后应默认折叠",
  );
  ...
  assert(
    planSteps && planSteps.length === 4,
    `plan 应有 4 步 (实际 ${planSteps ? planSteps.length : 0})`,
  );
  ...
  assert(
    w.document.querySelectorAll(".tool-card").length === 2,
    "错误不应清空已渲染的工具卡",
  );
  assert(
    w.document.querySelector(".plan-card") !== null,
    "错误不应清空 plan 卡",
  );
  assert(
    w.document.querySelectorAll(".text-stream-segment").length >= 1,
    "错误不应清空文本",
  );
  ...
  assert(
    lastSent && lastSent.type === "send_message",
    `bridge.send 应发 send_message, 实际 ${lastSent?.type}`,
  );
    ...
    references: [
      {
        name: "Foo.kt",
        path: "/abs/Foo.kt",
        relativePath: "src/main/kotlin/Foo.kt",
        size: 1024,
      },
    ],
  });
  ...
  assert(
    w.document
      ...
      .classList.contains("artifacts-collapsed"),
  ...
  assert(
    mailSent &&
      mailSent.type === "open_external_url" &&
      mailSent.url === "mailto:hi@example.com",
  ...
  assert(
    fileSent &&
      fileSent.type === "open_external_url" &&
      fileSent.url === "file:///tmp/test.kt",
  ...
  assert(
    nestedSent &&
      nestedSent.type === "open_external_url" &&
      nestedSent.url === "https://nested.example.com",
  ...
  const mockBridgeMod =
    await import("file:///tmp/e2e-webui/webui/js/bridge.js");
  ...
  // 1) 默认块间距 = 3px
  assert(
    /\.assistant-content\s*>\s*\*\s*\+\s*\*,[\s\S]*?margin-top:\s*2px/.test(
      mdCss,
    ),
    "markdown.css 默认块间距应为 2px (v2.3 与 Swing 端 MarkdownRenderer 一致)",
  );
  ...
  const segMatch = (chatCss.match(
    /\.text-stream-segment\s*\+\s*\.text-stream-segment\s*\{[^}]*margin-top:\s*(\d+)px/,
  ) || [])[1];
  assert(
    segMatch === "3",
    `text-stream-segment 间距应为 3px,实际 ${segMatch}px`,
  );
