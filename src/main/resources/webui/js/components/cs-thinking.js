...
const STATES = {
    running: {
        cls: "running",
        iconHtml: '<span class="thinking-dot thinking-dot-visual"></span>'.repeat(3),
        label: (ms) => `思考中 · ${(ms / 1000).toFixed(1)}s`,
    },
    completed: {
        cls: "completed",
        iconHtml: '<i class="fas fa-check" style="color:var(--success);font-size:11px;"></i>',
        label: (ms) => `已思考 ${(ms / 1000).toFixed(1)}s`,
    },
    collapsed: {
        cls: "completed",
        iconHtml: '<i class="fas fa-check" style="color:var(--fg-3);font-size:11px;"></i>',
        label: (ms) => `已思考 ${(ms / 1000).toFixed(1)}s`,
    },
};
...
export class Thinking {
     ...
     */
    constructor(opts = {}) {
        this.id = opts.id || ("thinking-" + Date.now() + "-" + Math.random().toString(36).slice(2, 7));
        this.state = "running";
